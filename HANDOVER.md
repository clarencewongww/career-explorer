...
43. **"What stood out" rows are all the same length** — `.reveal__topmark` was `width: fit-content`, so each row's width followed its word length ("No. 1 · R · Realistic" vs "No. 2 · I · Investigative") and read like ranking bars. Rows are now `width: 100%` on every viewport (moved up from the old ≤479px-only rule), so all three top-3 cards are identical full-width strips. Verified equal (±1px) at 375/768/1024/1440; tooltip placement still in-viewport; full regression green.
