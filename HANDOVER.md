...
> **Status:** Foundation + welcome + 24-item mixed-format quiz + fixed footer + intro greeting + light/dark mode + careers database (1,744 roles) + **reveal build complete** (real matching, career cards with country-agnostic role descriptions + day-in-life vignettes, copy migration, dark-mode card surfaces, desktop split-scroll, smart clip-aware tooltips) + **collating progress animation** (5s stamp-tile beat) + **keep & regenerate** (students keep cards and shuffle fresh matches) + **smooth shuffle** (compact desktop cards fit one view; mobile fresh-first + auto-scroll; stamp-in feedback) + **hosted on GitHub Pages**. Workbook Section 1 is next.
...
| **Keep & regenerate** — each card toggles kept (whole-card tap OR `.career-card__keep` stamp button, `aria-pressed`); `.reveal__shuffle` ("Show me different careers →") replaces every non-kept card with fresh picks excluding `revealKeptIds` + `revealSeenIds` (zero repeats across rounds); counter chip (aria-live "N of M kept"); all-kept → `.reveal__done` "All kept — you're set!"; exhausted pool → `.reveal__exhausted` hint; `toggleKeep`/`refreshReveal` re-render in place with scroll restoration; reset on quiz retake, preserved across workbook round-trip | ✅ Verified | Harness A–O: keep1→shuffle3→keep2→shuffle1→all-kept, 3-round zero-duplicate, keyboard, dark, mobile, reduced motion |
